...
- `src/context/AppContext.tsx` — также содержит runtime-конфигурацию `allowedRolesForSegmentation`
...
- Ограничение на создание сегментов по ролям тоже централизовано в `AppContext` через массив `allowedRolesForSegmentation`.
...
- создание сегмента дополнительно проверяет, что все сообщения в `draftSelection` имеют разрешенную роль
...
- `mocks/0b2a5a2a058e4df59b344416a45bc259.csv` — расширенный mock-диалог, на текущий момент `114` data-rows.
...
- Role-based segmentation: сообщения с ролями вне `allowedRolesForSegmentation` визуально помечаются как недоступные для разметки (`opacity: 0.5`, `cursor: not-allowed`), но selection не отключается полностью; блокировка происходит на этапе `createMarkFromDraftSelection`.
- Role-based segmentation для resize: корректировка границ существующего segment тоже ограничена непрерывным блоком разрешенных ролей; drag-resize не должен пересекать сообщение с запрещенной ролью даже как промежуточный участок.
...
- Edit mode layout behavior: при переходе в режим редактирования центральный блок не должен менять размер из-за `textarea`; высота редактируемой строки и `textarea` зафиксированы, лишний контент уходит во внутренний scroll поля.
...
- role-based segmentation: создавать segment можно только по сообщениям разрешенных ролей
- resize segment ограничен role barriers: нельзя растянуть segment через сообщение с неразрешенной ролью к следующему разрешенному сообщению
- progress tracking разметки по сообщениям с учетом role-filter
- edit mode с фиксированным layout центрального блока
- навигация к segment из правой панели с учетом chunk loading
...
- выделяет фрагмент текста или целое сообщение
...
- основной CSV mock расширен до 100+ сообщений для тестирования chunk rendering и поведения `Workspace` на длинном списке
...
- Логика role-based segmentation теперь влияет и на `updateMarkRange`; при изменении правил ролей нужно перепроверять не только создание сегментов, но и drag-resize существующих.
...
- В `Workspace` edit mode теперь завязан на фиксированную высоту `textarea`; если в будущем понадобится авто-рост поля, это может снова повлиять на layout и scroll behavior.
...
- Segment navigation завязана на `marks[index].selectedSegment[0]` и `activeMarkIndex`; любые изменения модели segment или порядка массива `marks` нужно проверять вместе с логикой прокрутки.
